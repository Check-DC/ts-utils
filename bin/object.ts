 ...
 */
export const parseJSON = <T>(str: string, defaultVal: any = []): T => {
  try {
    if (str != null)
 ...
 */
export const deepClone = <T extends Record<string, any>>(obj: T | Record<string, any>): T => parseJSON(JSON.stringify(obj), {})
 ...
 */
export const sanitizeQuery = (obj: Record<string, any>): Record<string, any> => parseNumValues(removeEmptyKeys(obj))
 ...
 */
export const removeKeys = (obj: Record<string, any>, keys: string[]): Record<string, any> => {
  if (obj) {
    obj = deepClone(obj)
    keys.forEach((key) => {
 ...
 */
export const renameKey = (obj: { [key: string]: any }, oldKey: string, newKey: string) => {
 ...
 */
export const renameKeys = (obj: { [key: string]: any }, oldKeys: string[], newKeys: string[]) => {
...
export const querylize = (obj: Record<string, any>  | undefined) => {
  ...
  const str: string[] = Object.entries(obj)
    ...
    .map(([prop, value]) => `${encodeURIComponent(prop)}=${encodeURIComponent(value)}`)
...
export const parseNumber = (obj: Record<string, any>): Record<string, any> => {
  ...
  Object.keys(obj).forEach((key) => {
    if (obj[key])
      res[key] = !Number.isNaN(obj[key]) ? Number(obj[key]) : obj[key]
  })
  return res
}

export const reserveKeys = (obj: Record<string, any>, keys: string[]): Record<string, any> => {
...
export const hasKeysAndValues = <T>(obj: T, keys: (keyof T)[]): boolean => {
  if (obj) {
    return keys.every(
      key => Object.prototype.hasOwnProperty.call(obj, key) && obj[key] !== undefined && obj[key] !== null,
    )
  }
